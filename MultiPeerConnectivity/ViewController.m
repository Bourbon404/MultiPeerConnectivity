...
- (void)viewDidLoad{
    [super viewDidLoad];
    self.peelId = [[[MCPeerID alloc] initWithDisplayName:[[UIDevice currentDevice] name]]autorelease];
...
-(IBAction)serverValueChange:(id)sender{
    ...
    if (_serverSwitch.on) {
        if (nil == _advertiserAssistant) {
            self.advertiserAssistant = [[[MCAdvertiserAssistant alloc] initWithServiceType:SERVICE_TYPE discoveryInfo:nil session:_session] autorelease];
...
-(IBAction)clientStart:(id)sender{
    if (nil == _browser) {
        self.browser = [[[MCBrowserViewController alloc] initWithServiceType:SERVICE_TYPE session:_session] autorelease];
...
-(void)dealloc{
    [_serverSwitch release];
    [_clientStartButton release];
    [_clientStopButton release];
    [_peelId release];
    [_session release];
...
- (IBAction)sendMsg:(id)sender
{
    ...
    BOOL connect = [self.session sendData:strData toPeers:peerIDArray withMode:(MCSessionSendDataReliable) error:nil];
